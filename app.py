...
def create_app():
    ...
    if torch_import and SERPER_KEY:
        try:
            ...
            llm_pipeline = pipeline("text-generation", model=model, tokenizer=tokenizer, max_new_tokens=400, do_sample=True, temperature=0.6)
    ...
    def process_pdf_api():
        ...
        print(f"[{timestamp}] [INFO] 요청 데이터: folderId='{folder_id}', fileName='{file_name}'")

        if not folder_id or not file_name:
            return jsonify({"status": "error", "message": "folderId와 fileName은 필수입니다."}), 400
        ...
        try:
            ...
            if deadline_str:
                try:
                    date_match = re.search(r'(\d{4}년\s*\d{1,2}월\s*\d{1,2}일)', deadline_str)
                    if date_match:
                        clean_deadline_str = date_match.group(1).replace(" ", "")
                        company.deadline = datetime.strptime(clean_deadline_str, '%Y년%m월%d일').date()
                    else: company.deadline = None
                except ValueError: company.deadline = None
            else: company.deadline = None
            ...
            # --- [중요] DB 저장 직전 데이터 로깅 ---
            company_data_to_save = {k: v for k, v in company.__dict__.items() if not k.startswith('_')}
            job_data_to_save = {k: v for k, v in job_posting.__dict__.items() if not k.startswith('_')}
            ...
            return jsonify({"status": "success", "message": f"'{file_name}' 파일이 성공적으로 처리되어 데이터베이스에 저장되었습니다."}), 201
